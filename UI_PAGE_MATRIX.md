...
| Participants / `/Admin/Events/Participants/{id}` | Admin manages signup settings and participants | Full-width data/table + accepted detail dialog | `Participants.cshtml`, `Participant.cshtml`; participant table/dialog classes | Compact settings, search/status controls, current/history groups, route fallback | Accepted participant-detail dialog states; page-local table markup | Approved — Manage and Add Participant user manual approval, 2026-09-08 | Manage Remove/Restore/Transfer ownership auto-scroll correction accepted; focused regression/source review clear. Preserve guards, layout and separate page approvals |
