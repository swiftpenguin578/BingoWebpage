...
public sealed class AdminShellUiTests
{
    ...
    public void AdminPagesUseTheDedicatedShellAndRealNavigationDestinations()
    {
        ...
        Assert.Contains("Community Operations", layout);
        ...
        Assert.Contains("--admin-page: #0b1018", styles);
        Assert.Contains("--admin-surface: #111823", styles);
        Assert.Contains(".admin-event-nav::before", styles);
        Assert.Contains("--admin-event-spine-center: 0.6375rem", styles);
        Assert.Contains("--admin-event-branch-length: 0.75rem", styles);
        Assert.Contains("--admin-event-connector-gap: 0.25rem", styles);
        Assert.Contains("--admin-event-link-left", styles);
        Assert.Contains("var(--admin-event-branch-end)", styles);
