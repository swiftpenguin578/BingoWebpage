...
class FormDataMock {
  constructor(form) {
    this.values = [];
    if (form) form.querySelectorAll("input, select").forEach(control => { if (control.name) this.set(control.name, control.value); });
  }
...
const createRoot = () => {
  ...
  scope.append(new Node("select", { name: "eventId", value: "event-1" }));
  const create = new Form("form", { action: "/Admin/Accounts/Create", method: "post" });
  create.append(new Node("input", { name: "Input.Username", value: "test-account" }));
...
const nested = () => body.querySelector("dialog.admin-account-confirmation-dialog");
...
(async () => {
  ...
  assert.equal(new URL(window.location.href).pathname, "/Admin/Accounts", "successful Create settles on the canonical Accounts directory");
  ...
  assert.equal(nested().open, true, "Emergency action opens a second native dialog");
  ...
  assert.equal(nested().open, false, "nested Escape closes only confirmation");
  ...
  assert.equal(nested().open, false, "nested backdrop closes only confirmation");
  emergency.dispatch("click");
  nested().querySelector("[data-account-confirmation-cancel]").dispatch("click");
  assert.equal(nested().open, false, "Cancel closes only confirmation");
  ...
  assert.equal(requested.at(-1).method, "POST");
  assert.match(requested.at(-1).url, /handler=GenerateEmergencyLink/);
  assert.equal([...requested.at(-1).body].some(([name, value]) => name === "overlay" && value === "1"), true, "Emergency confirm posts overlay=1");
  ...
  assert.equal(nested().open, true, "website reset action uses the shared native confirmation dialog");
  ...
  await flush();
  assert.equal(nested().open, false, "website reset confirmation closes after the rendered response");
  ...
  assert.equal(nested().open, true, "website disable action uses the shared native confirmation dialog");
  ...
  nested().querySelector("[data-account-confirmation-cancel]").dispatch("click");
  assert.equal(nested().open, false, "website disable Cancel closes only the confirmation");
