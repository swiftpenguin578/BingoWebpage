...
A public-username change must select an active `AccountOsrsCharacter` link and win the same case-insensitive uniqueness constraint. It also changes the password-login username. The application updates `public_username_snapshot` only for that account's participants in events whose signups are currently open. Closed and historical event snapshots remain unchanged.
