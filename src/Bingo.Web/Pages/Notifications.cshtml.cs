...
public sealed class NotificationsModel(Bingo.Infrastructure.Persistence.ApplicationDbContext db, TimeProvider time, IStringLocalizer<SharedResource> text) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid? read, CancellationToken cancellationToken)
    {
        ...
        Notifications = await db.PersonalNotifications.AsNoTracking().Where(item => item.RecipientAccountId == accountId).OrderByDescending(item => item.CreatedAt).Select(item => new NotificationView(item.Id, item.Title, item.Route, item.CreatedAt, item.ReadAt)).ToListAsync(cancellationToken);
        return Page();
    }
    ...
    public string Title(string type) => type switch { "account.admin_granted" => text["Admin access granted"], "account.admin_revoked" => text["Admin access revoked"], "account.restored" => text["Account restored"], _ => text["Notifications"] };
    public string Detail(string type) => type switch { "account.admin_granted" => text["An administrator granted your account Admin access."], "account.admin_revoked" => text["An administrator removed your Admin access."], "account.restored" => text["An administrator restored your account."], _ => string.Empty };
    public sealed record NotificationView(Guid Id, string Type, string Route, DateTimeOffset CreatedAt, DateTimeOffset? ReadAt);
}
