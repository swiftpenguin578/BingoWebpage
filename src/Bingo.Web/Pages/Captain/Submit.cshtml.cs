...
public sealed class SubmitModel(ApplicationDbContext db, ISubmissionService submissions, IEvidenceAuthority evidenceAuthority, TimeProvider time, IStringLocalizer<SharedResource> text, ILogger<SubmitModel> logger) : PageModel
{
    ...
    private async Task<bool> Load(Guid? tileId, Guid? requestedEventId, Guid? requestedTeamId, CancellationToken ct)
    {
        ...
        var tileContributionByItem = await (from contribution in db.SubmissionContributions.AsNoTracking()
                                            join drop in db.BoardRequirementDropSnapshots.AsNoTracking() on contribution.DropSnapshotId equals drop.Id
                                            where contribution.TeamId == Scope.TeamId && requirementIds.Contains(contribution.RequirementId) && contribution.ReversedAt == null
                                            group contribution by new { contribution.RequirementId, drop.ItemIdSnapshot } into grouped
                                            select new { grouped.Key.RequirementId, grouped.Key.ItemIdSnapshot, Total = grouped.Sum(value => value.Amount) })
            .ToDictionaryAsync(value => (value.RequirementId, value.ItemIdSnapshot), value => value.Total, ct);
        var availableTileIds = tileRequirements.GroupBy(value => value.BoardTileId).Where(group => group.Any(requirement =>
        {
            if (tileContributions.GetValueOrDefault(requirement.Id) >= requirement.TargetContribution) return false;
            if (requirement.ManualObjective) return true;
            return tileDrops.Where(drop => drop.RequirementId == requirement.Id).Any(drop =>
            {
                var maximum = drop.MaximumContribution ?? (requirement.DuplicatesAllowed ? int.MaxValue : 1);
                var used = requirement.DuplicatesAllowed
                    ? tileContributionRows.GetValueOrDefault((requirement.Id, drop.Id))
                    : tileContributionByItem.GetValueOrDefault((requirement.Id, drop.ItemIdSnapshot));
                return maximum > used;
            });
        ...
        var requirements = await db.BoardRequirementSnapshots.AsNoTracking().Where(x => x.BoardTileId == tileId).OrderBy(x => x.Position).ToListAsync(ct); var ids = requirements.Select(x => x.Id).ToList(); var drops = await db.BoardRequirementDropSnapshots.AsNoTracking().Where(x => ids.Contains(x.RequirementId)).OrderBy(x => x.BossName).ThenBy(x => x.ItemName).ToListAsync(ct); var contributions = await db.SubmissionContributions.AsNoTracking().Where(x => x.TeamId == Scope.TeamId && ids.Contains(x.RequirementId) && x.ReversedAt == null).ToListAsync(ct); var dropById = drops.ToDictionary(x => x.Id); var contributed = contributions.GroupBy(x => x.RequirementId).ToDictionary(x => x.Key, x => x.Sum(y => y.Amount)); var contributedByDrop = contributions.Where(x => x.DropSnapshotId is not null).GroupBy(x => x.DropSnapshotId!.Value).ToDictionary(x => x.Key, x => x.Sum(y => y.Amount)); var contributedByItem = contributions.Where(x => x.DropSnapshotId is not null && dropById.ContainsKey(x.DropSnapshotId.Value)).GroupBy(x => (x.RequirementId, dropById[x.DropSnapshotId!.Value].ItemIdSnapshot)).ToDictionary(x => x.Key, x => x.Sum(y => y.Amount)); Requirements = requirements.Select(x => new RequirementView(x.Id, x.Description, x.TargetContribution, Math.Max(0, x.TargetContribution - contributed.GetValueOrDefault(x.Id)), x.ManualObjective, x.DuplicatesAllowed, drops.Where(d => d.RequirementId == x.Id && (d.MaximumContribution ?? (x.DuplicatesAllowed ? int.MaxValue : 1)) > (x.DuplicatesAllowed ? contributedByDrop.GetValueOrDefault(d.Id) : contributedByItem.GetValueOrDefault((x.Id, d.ItemIdSnapshot)))).Select(d => new DropView(x.Id, d.Id, d.BossName, d.ItemName, d.DisplayRate, d.CreditedWeight)).ToList())).ToList(); return true;
    }
