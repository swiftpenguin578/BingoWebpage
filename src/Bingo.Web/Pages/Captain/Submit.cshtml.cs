...
public sealed class SubmitModel(ApplicationDbContext db, ISubmissionService submissions, IStringLocalizer<SharedResource> text, ILogger<SubmitModel> logger) : PageModel
{
    ...
    public async Task<IActionResult> OnGetDrawerAsync(Guid tileId, CancellationToken ct) => await Load(tileId, ct) ? Partial("_SubmissionDrawer", this) : NotFound();
    public async Task<IActionResult> OnPostAsync(Guid tileId, CancellationToken ct)
    {
        if (!await Load(tileId, ct)) return NotFound(); if (Input.Evidence is null) ModelState.AddModelError("Input.Evidence", text["Paste, drag, or choose one screenshot."]); if (!ModelState.IsValid) return Page();
        try { await using var stream = Input.Evidence!.OpenReadStream(); var result = await submissions.CreateAsync(new(User.GetAccountId()!.Value, User.GetEventId()!.Value, User.GetTeamId()!.Value, tileId, Input.RequirementId, Input.DropSnapshotId, Input.CreditedParticipantId, Input.ClaimedWeight, Input.Note, Input.Evidence.FileName, stream, Input.RequestPublicPrivacy), ct); TempData["StatusMessage"] = text["Evidence submitted for review."]; return RedirectToPage("Submission", new { id = result.SubmissionId }); }
    ...
    public async Task<IActionResult> OnPostDrawerAsync(Guid tileId, CancellationToken ct)
    {
        if (!await Load(tileId, ct)) return NotFound();
        ...
        try
        {
            ...
            var result = await submissions.CreateAsync(new(User.GetAccountId()!.Value, User.GetEventId()!.Value, User.GetTeamId()!.Value, tileId, Input.RequirementId, Input.DropSnapshotId, Input.CreditedParticipantId, Input.ClaimedWeight, Input.Note, Input.Evidence.FileName, stream, Input.RequestPublicPrivacy), ct);
