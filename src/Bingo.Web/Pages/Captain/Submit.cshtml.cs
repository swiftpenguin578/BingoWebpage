...
public sealed class SubmitModel(ApplicationDbContext db,ISubmissionService submissions) : PageModel
{
    ...
    public sealed record TileDetails(Guid Id,string Name,string Description,string EvidenceInstructions);public sealed record PlayerView(Guid Id,string Name);public sealed record RequirementView(Guid Id,string Description,int Target,int Remaining,bool Manual,bool HigherWeights,bool DuplicatesAllowed,IReadOnlyList<DropView>Drops);public sealed record DropView(Guid Id,string Boss,string Item,string Rate);
}
