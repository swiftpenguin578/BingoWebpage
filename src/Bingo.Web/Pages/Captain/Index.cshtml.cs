...
public sealed class IndexModel(ApplicationDbContext db, TimeProvider time) : PageModel
{
    ...
    public async Task OnGetAsync(CancellationToken ct)
    {
        ...
        Submissions = await (from s in db.Submissions.AsNoTracking() join t in db.BoardTiles on s.BoardTileId equals t.Id join p in db.PrimaryCharacters() on s.CreditedParticipantId equals p.ParticipantId where s.TeamId == teamId orderby s.SubmittedAt descending select new SubmissionView(s.Id, t.NameSnapshot, p.Name, s.Status, s.ClaimedWeight, s.ApprovedContribution, s.SubmittedAt, s.CurrentReviewerNote)).Take(100).ToListAsync(ct);
    }
