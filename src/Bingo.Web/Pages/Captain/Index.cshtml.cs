    ...
    IStringLocalizer<SharedResource> text) : PageModel
{
    ...
    {
        var scope = await ResolveOperatorAsync(eventId, teamId, cancellationToken);
        if (scope is null) return NotFound();
        if (!await LoadAsync(scope, cancellationToken)) return NotFound();
    ...
    public async Task<IActionResult> OnGetLedgerAsync(Guid? eventId, Guid? teamId, string? search, Guid? player, int ledgerPage = 1, CancellationToken cancellationToken = default)
    {
        ...
        return Partial("_SubmissionLedger", Ledger);
    }
    ...
    private async Task<EvidenceActorScope?> ResolveOperatorAsync(Guid? requestedEventId, Guid? requestedTeamId, CancellationToken cancellationToken)
    {
        ...
        if (scope.Kind is not (EvidenceActorKind.Captain or EvidenceActorKind.EmergencyCaptain)) return null;

        var eventItem = await db.Events.AsNoTracking()
            .SingleOrDefaultAsync(value => value.Id == scope.EventId && value.State == EventState.Live, cancellationToken);
        var team = await db.Teams.AsNoTracking()
            .SingleOrDefaultAsync(value => value.Id == scope.TeamId && value.EventId == scope.EventId && value.Active, cancellationToken);
        return eventItem is null || team is null ? null : scope;
    }
    ...
    private async Task<bool> LoadAsync(EvidenceActorScope scope, CancellationToken cancellationToken)
    {
        ...
        var eventItem = await db.Events.AsNoTracking().SingleAsync(value => value.Id == scope.EventId, cancellationToken);
        var team = await db.Teams.AsNoTracking().SingleAsync(value => value.Id == scope.TeamId && value.EventId == scope.EventId, cancellationToken);
