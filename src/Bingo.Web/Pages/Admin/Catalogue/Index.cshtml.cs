...
public sealed class IndexModel(ApplicationDbContext dbContext, IAuditWriter auditWriter, TimeProvider timeProvider) : PageModel
{
    ...
    public async Task<IActionResult> OnPostUpdateBossAsync(Guid recordId, string name, string category, decimal? efficientRate, string? dataSource, string? imageUrl, CancellationToken ct)
    {
        ...
        foreach (var drop in drops) drop.Update(drop.DisplayRate, drop.NumericProbability, drop.RateConditionNote, efficientRate is > 0 && drop.NumericProbability is > 0 ? 1 / (efficientRate.Value * drop.NumericProbability.Value) : null, drop.DataSource, timeProvider.GetUtcNow());
    ...
    public async Task<IActionResult> OnPostUpdateDropAsync(Guid recordId, string itemName, string displayRate, string originalDisplayRate, decimal? numericProbability, decimal? originalNumericProbability, string? dataSource, string? imageUrl, bool useExistingItem, CancellationToken ct)
    {
        ...
        var parsedProbability = DropRateParser.TryParseProbability(cleanDisplayRate);
        var probability = numericProbabilityChanged
            ? numericProbability
            : displayRateChanged && parsedProbability is > 0
                ? parsedProbability
                : numericProbability ?? parsedProbability;
        var bossRate = await dbContext.BossActivities.Where(x => x.Id == entity.BossActivityId).Select(x => x.EfficientCompletionsPerHour).SingleAsync(ct); decimal? calculatedEhb = bossRate is > 0 && probability is > 0 ? 1 / (bossRate.Value * probability.Value) : null;
    ...
    private async Task LoadAsync(CancellationToken ct)
    {
        ...
        var drops = await (from d in dbContext.SourceDrops.AsNoTracking() join i in dbContext.CatalogueItems on d.ItemId equals i.Id select new DropRow(d.Id, d.BossActivityId, i.Name, d.DisplayRate, d.NumericProbability, d.DefaultEhbEstimate, d.Active, d.DataSource, i.ImageUrl, d.DataUpdatedAt, Array.Empty<RateVariantRow>())).ToListAsync(ct);
    ...
    public sealed record DropRow(Guid Id, Guid BossActivityId, string ItemName, string DisplayRate, decimal? Probability, decimal? DefaultEhb, bool Active, string? DataSource, string? ImageUrl, DateTimeOffset UpdatedAt, IReadOnlyList<RateVariantRow> RateVariants);
