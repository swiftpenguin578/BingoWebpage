...
public sealed class IndexModel(ApplicationDbContext db) : PageModel
{
    ...
    public async Task OnGetAsync(Guid? eventId, Guid? teamId, Guid? tileId, SubmissionStatus? status, CancellationToken ct)
    {
        ...
        var query = from s in db.Submissions.AsNoTracking() join team in db.Teams on s.TeamId equals team.Id join tile in db.BoardTiles on s.BoardTileId equals tile.Id join player in db.PrimaryCharacters() on s.CreditedParticipantId equals player.ParticipantId where s.EventId == ev.Id select new { s, team, tile, player }; if (teamId is not null) query = query.Where(x => x.s.TeamId == teamId); if (tileId is not null) query = query.Where(x => x.s.BoardTileId == tileId); if (status is not null) query = query.Where(x => x.s.Status == status); Rows = await query.OrderByDescending(x => x.s.SubmittedAt).Select(x => new Row(x.s.Id, x.s.SubmittedAt, x.s.SubmittedAt > ev.EventEndsAt, x.team.Name, x.tile.NameSnapshot, x.player.Name, x.s.Status, x.s.ClaimedWeight, x.s.ApprovedContribution, x.s.ExpectedEvidenceCode, x.s.CurrentReviewerNote)).ToListAsync(ct);
    }
