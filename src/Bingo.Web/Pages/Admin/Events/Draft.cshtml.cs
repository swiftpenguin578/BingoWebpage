...
public sealed class DraftModel(ApplicationDbContext db, TimeProvider time, IAuditWriter audit, CaptainAccountProvisioner captainAccounts, IAdminCollaborationNotifier collaboration, ISignupService signupService) : PageModel
{
    ...
    { if(await db.Events.AnyAsync(x=>x.Id==id&&x.DraftLocked,ct)){TempData["StatusMessage"]="Website signup assignments are locked because the draft has started.";return RedirectToPage(new{id});}if(participantId is null){TempData["StatusMessage"]="Choose a participant.";return RedirectToPage(new{id});}var participantName=await db.EventParticipants.Where(x=>x.Id==participantId.Value&&x.EventId==id).Select(x=>x.PrimaryAccountName).SingleOrDefaultAsync(ct);if(participantName is null)return NotFound();var teamName=await AddMembership(id,teamId,participantId.Value,role,reason,null,ct);SetStatus($"{participantName} added to {teamName}.",UiMessageType.Success);return RedirectToPage(new{id}); }
