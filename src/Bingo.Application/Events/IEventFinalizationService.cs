...
public interface IEventFinalizationService
{
    ...
    Task FinalizeAsync(Guid eventId, LifecycleActor actor, CancellationToken ct = default);
    Task UnfinalizeAsync(Guid eventId, string reason, bool confirmed, LifecycleActor actor, CancellationToken ct = default);
