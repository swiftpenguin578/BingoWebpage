...
public sealed class SubmissionService(ApplicationDbContext db, IEvidenceStorage storage, TimeProvider time, IProgressNotifier? progressNotifier = null) : ISubmissionService
{
    public async Task<SubmissionResult> CreateAsync(CreateSubmissionCommand command, CancellationToken cancellationToken = default)
    {
        ...
        await ValidateTarget(command.EventId, command.TeamId, command.BoardTileId, command.RequirementId, command.DropSnapshotId, command.CreditedParticipantId, command.ClaimedWeight, cancellationToken);
        ...
        var submission = new Submission(Guid.NewGuid(), command.EventId, command.TeamId, command.BoardTileId, command.RequirementId, command.DropSnapshotId, command.CreditedParticipantId, command.ActorAccountId, command.ClaimedWeight, now, command.CaptainNote, expectedCode, command.RequestPublicPrivacy);
    ...
    public async Task<SubmissionResult> CorrectAsync(CorrectSubmissionCommand command, CancellationToken cancellationToken = default)
    {
        ...
        await ValidateTarget(submission.EventId, submission.TeamId, command.BoardTileId, command.RequirementId, command.DropSnapshotId, command.CreditedParticipantId, command.ClaimedWeight, cancellationToken);
        ...
        try
        {
            ...
            submission.EditPending(command.BoardTileId, command.RequirementId, command.DropSnapshotId, command.CreditedParticipantId, command.ClaimedWeight, command.CaptainNote, command.RequestPublicPrivacy);
    ...
    { await EnsureAdmin(command.AdminAccountId,cancellationToken);var s=await db.Submissions.SingleAsync(x=>x.Id==command.SubmissionId,cancellationToken);await ValidateTarget(s.EventId,s.TeamId,command.BoardTileId,command.RequirementId,command.DropSnapshotId,command.CreditedParticipantId,command.ClaimedWeight,cancellationToken);var before=Snapshot(s);s.EditPending(command.BoardTileId,command.RequirementId,command.DropSnapshotId,command.CreditedParticipantId,command.ClaimedWeight,s.CaptainNote);db.ReviewActions.Add(Action(s.Id,ReviewActionType.EditMetadata,command.AdminAccountId,time.GetUtcNow(),command.Note,before,Snapshot(s)));await db.SaveChangesAsync(cancellationToken); }
    ...
    private async Task ValidateTarget(Guid eventId,Guid teamId,Guid tileId,Guid requirementId,Guid? dropId,Guid participantId,int weight,CancellationToken cancellationToken)
    {
        if(weight<1)throw new InvalidOperationException("Contribution must be at least 1.");if(!await db.Teams.AnyAsync(x=>x.Id==teamId&&x.EventId==eventId&&x.Active,cancellationToken))throw new InvalidOperationException("Team not found.");if(!await db.TeamMemberships.AnyAsync(x=>x.TeamId==teamId&&x.EventParticipantId==participantId&&x.LeftAt==null,cancellationToken))throw new InvalidOperationException("The credited player does not belong to this team.");
        ...
        var requirement=await db.BoardRequirementSnapshots.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==requirementId&&x.BoardTileId==tile.Id,cancellationToken)??throw new InvalidOperationException("Choose a requirement from that tile.");if(weight>1&&!requirement.AllowHigherWeightings)throw new InvalidOperationException("This requirement does not allow a contribution above 1.");
        if(requirement.ManualObjective&&dropId is not null)throw new InvalidOperationException("Manual objectives do not use a drop.");if(!requirement.ManualObjective&&dropId is null)throw new InvalidOperationException("Choose an eligible drop.");if(!requirement.ManualObjective&&dropId is Guid selectedDropId){var drop=await db.BoardRequirementDropSnapshots.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==selectedDropId&&x.RequirementId==requirementId,cancellationToken)??throw new InvalidOperationException("Choose an eligible drop.");var maximum=drop.MaximumContribution??(requirement.DuplicatesAllowed?int.MaxValue:1);var dropApproved=await db.SubmissionContributions.Where(x=>x.TeamId==teamId&&x.RequirementId==requirementId&&x.DropSnapshotId==selectedDropId&&x.ReversedAt==null).SumAsync(x=>(int?)x.Amount,cancellationToken)??0;if(dropApproved>=maximum)throw new InvalidOperationException("This drop has already reached its approved contribution limit.");}
        var approved=await db.SubmissionContributions.Where(x=>x.TeamId==teamId&&x.RequirementId==requirementId&&x.ReversedAt==null).SumAsync(x=>(int?)x.Amount,cancellationToken)??0;if(approved>=requirement.TargetContribution)throw new InvalidOperationException("This requirement is already complete.");
    }
