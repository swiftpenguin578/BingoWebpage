...
- Catalogue display rates now use consistent input-friendly notation: multiple rolls use multiplier-first forms such as `7 × 1/2448`, reviewed personal rates display only the rate while their assumptions remain in the condition note, and future Wiki imports preserve multiplier-first notation. The seven calculable Expert ToA purple rates display rounded `1/x` equivalents while retaining their exact decimal probabilities. Grotesque Guardians' Adamant boots now has the missing two-roll numeric mechanics. Twenty-two active `(+1 variants)` display markers remain intentionally unresolved (27 including inactive rows); they were not flattened or guessed. Catalogue and public-board Wiki artwork now use a shared same-origin persistent cache instead of browser hotlinks. Original Wiki URLs remain authoritative; development files live under Git-ignored `data/catalogue-images`, production can mount `CatalogueImageCache:LocalPath`, and `--sync-catalogue-images` prewarms reviewed images at a configurable 500 ms default interval with backoff for `429`/`503` responses. Legacy Wiki image routes normalize to the working `/w/Special:Redirect/file` form, and future imports generate that form directly. The user supplied replacements for the 16 missing boss/activity images and two missing item images; they are recorded in the authoritative snapshot and applied locally, with CoX/CM and ToB/HM intentionally sharing artwork. The final local prewarm cached all 374 distinct current sources with zero failures and no rate limiting.
...
Review and refine the new Pass 12 flagship event-dashboard foundation before continuing through the public team-board and tile-detail states. The public overview now has real top-level View bingo, Recent drops, and Leaderboards views; Recent drops is populated from approved submissions with privacy suppression, Drop EHB uses approved credited contributions, bingo standings remain visible, and Activity EHB is explicitly presented as awaiting the deferred Wise Old Man integration. Evidence review and the administrative fallback submission form are now explicitly event-scoped: Manage Event carries its event into the queue, direct queue access requires an explicit choice when multiple events are eligible, and the selected event is retained when opening the submission form. The admin notification inbox now aggregates pending submissions from every live/final-review event, includes the event name in each entry, and subscribes to every eligible event's progress invalidations; captain notifications remain limited to their own event.

The approved direction is now recorded as the site-wide flagship visual system in `UI_OVERHAUL_ROADMAP.md` section 3.6. The **View bingo** state at `/Events/test-15-dkl-live/Board` is the visual source of truth for the entire overhaul, including later public, captain, signup, authentication, error, account, and administration work. The section records the reference composition, exact spacing scale, density, colour tokens, typography, surfaces, outlines, radii, controls, tables, dialogs, complete/in-progress/ranking states, breakpoints, responsive behavior, and motion. Exact implemented values remain centralized under `body.public-event-shell` and related shared selectors in `wwwroot/css/site.css`; later passes must promote and reuse shared tokens/components rather than copying page-local styles or retaining a separate legacy admin theme.

The Pass 12 team-specific board now has an initial flagship implementation awaiting user approval. Desktop overview cards progressively open a route-backed modal overlay while keeping the overview mounted and faded underneath; the address bar changes to the real team-board URL, and close, backdrop, Escape, and browser Back return to the overview. The overlay intentionally contains only the height-matched statistics sidebar, detailed grid, and close control—no global shell, team masthead, or previous/next navigation. Direct team URLs retain the standalone page, and widths below `761px` use immediate ordinary navigation with no overlay or transition. The earlier cross-document View Transition experiment was removed because browser-generated snapshots produced unreliable forward animation. The board uses square detailed tiles, progress/complete outlines, contained mobile board scrolling, and artwork selection that prefers the tile's published custom image snapshot before falling back to as many as four distinct catalogue boss images from its published requirements. Three images use a lowered two-over-one cluster and four use a lowered overlapping 2×2 cluster. Artwork is deduplicated by encounter family for the three Wilderness pairs, CoX/CM, ToB/HM, Nightmare/Phosani's, ToA/Expert ToA, and Gauntlet/Corrupted Gauntlet. This required no migration. The focused Web release build passes with zero warnings or errors; desktop overlay open/close/history, compact overlay content, and standalone direct-route behavior were browser-verified. User visual approval remains outstanding.

The team board now includes a height-matched statistics sidebar. On wide screens it matches the 5×5 board grid and contains bingo completion, structurally matched Drop EHB and Wise Old Man Activity EHB sections with team-total/MVP positions, and the full public team contribution ranking. The team ranking includes every active roster member even when all values are zero, while the event-wide player leaderboard remains limited to players with public approved submissions. Drop EHB retains a green MVP value as an intentional highlight; Activity EHB remains an honest unavailable state until Wise Old Man synchronization exists. Test 15 distributes its representative approvals across all ten team members so this layout and the player leaderboard use realistic data. At `1280px`, the measured sidebar is approximately `720px` high beside a `715px` board with no page overflow. At `800px`, the three main sections form equal columns above the board and the contribution list hides. At `430px`, the sections stack, the page remains free of horizontal overflow, and only the detailed board uses contained horizontal scrolling. These desktop, intermediate, and narrow browser checks passed; user visual approval remains outstanding.

TEST 15 combines the published 5×5 DKL board with the 60-player large-draft pool: six finalized drafted teams of ten, six captain accounts, and a live event. The reset seed was exercised against PostgreSQL and every team was verified to contain exactly ten members. Combined-source fixture names use `/` (for example, `Barrows / Moons`) instead of `+`.

The Test 15 EHB discrepancy is resolved. The original fixture assigned each requirement's entire target to its first eligible drop, and the public calculation summed standalone time-to-specific-drop values, producing `4419.35 EHB`. That was incorrect for objectives such as “any six Theatre of Blood purples,” because the same raids roll all eligible alternatives together. Team and player **Drop EHB** now use the same expected-objective allocation: each approved contribution receives its proportional share of the tile's immutable combined EHB snapshot, weighted drops receive multiple shares, a completed tile equals exactly its tile EHB, and a completed board equals its board EHB. Specific-component objectives such as Voidwaker remain longer because their combined tile calculation requires those specific pieces. Test 15 now creates representative individual approvals across eligible drops while honoring credited weights, duplicate rules, and per-drop limits. Its completed team has `98` approvals and calculates to `741.0829`; public summaries round this to the scan-friendly whole-number display `741 EHB`. The separately stored board total is `741.0832`, with the `0.0003` raw difference caused by summing individually four-decimal tile snapshots. The underlying precision remains authoritative for calculations and ranking. The public overview no longer shows the redundant `Leading` badge. The development reset completed successfully, the Web release build passed with 0 warnings and 0 errors, and all 13 focused submission-workflow integration tests passed, including new team/player EHB consistency coverage.

1. Obtain user approval for the seeded event dashboard at desktop, intermediate, and narrow widths, including all three top-level views.
2. Bring the team-board and tile-detail states into the same flagship visual treatment while preserving navigation context, evidence privacy, and SignalR behavior.
3. Return to the captain submission pass afterward and reuse the approved public board treatment. The deferred team-only focus feature covers private tile, row, and column priorities.
