...
- **S8-01 — Historical acceptance superseded only for route ownership.** Participant history now uses the planned neutral `/Submissions` workspace and `/Submissions/{id}` detail route; Captain history remains in its separately approved team-scoped workspace.
...
- [ ] Verify `/Submissions` exposes the signed-in current team member's complete retained team ledger, including departed credited members, and normal authenticated navigation reaches it. From the team-specific Board main progress sidebar, Captains/co-captains retain the existing `Team history` button to `/Captain`, while ordinary participants see the same button treatment targeting `/Submissions`. Pending/rejected mutations are owner-only; pending owner rows permit edit, screenshot replacement, and withdrawal through cutoff; rejected owner rows permit only the existing linked resubmission; teammate-owned and all other states are read-only.
- [ ] Open an evidence rejection notification as the credited participant and as an eligible Captain/co-captain recipient. Both must mark the notification read and resolve through their authorized stored route (`/Submissions/{id}` for the credited owner, `/Captain/Submissions/{id}` for the Captain/co-captain); owner routing wins when one account has both roles, and no route exposes another team's private evidence.
