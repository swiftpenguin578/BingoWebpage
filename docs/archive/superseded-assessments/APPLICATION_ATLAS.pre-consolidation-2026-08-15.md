...
- **Historical F-01 (resolved):** both producers previously emitted `/Admin/Events/Teams/{eventId}`, but no such Razor page exists. They now emit the existing team/authority workspace `/Admin/Events/Draft/{eventId}`.
...
| `SignupClosed → Live` | Admin Start event now or one scheduled attempt | `/Admin/Events/Manage/{id}` | `IEventLifecycleService` / `EventLifecycleService`; `EventStatePolicy` | Manual/successful scheduled start uses actual transition time; scheduled attempt retains configured instant separately. | Normal cutoff remains configured end +30m. | Finalized draft, published board, valid Playing assignments, usable Captain/emergency, valid schedule, no current event. Direct routes are Schedule, Draft, Board, Participant; missing-Captain resolution now routes to Draft (F-01 resolved). | Event version, singleton advisory lock, unique scheduled attempt. Blocked scheduled start never auto-retries; Admin resolves and starts manually. |
...
- User-facing lifecycle classification is nevertheless spread across the filter, `BingoEvent`, PageModels, display projections and services. F-03 recommends one guidance projection without weakening those checks. The `TEAM_ACCESS_MISSING` destination remains duplicated between the lifecycle service and Manage mapping; focused tests now keep both on Draft after F-01’s correction.
...
| `/Admin/Events/Teams/{id}` missing-Captain route | Historical defect (resolved) | Replaced in both independent producers by existing `/Admin/Events/Draft/{id}`; retained here as F-01 history. |
...
| --- | --- | --- | --- | --- |
| F-01 | High (resolved) | Missing-Captain start-readiness resolution route was unreachable. | Before correction, both producers emitted `/Admin/Events/Teams/{id}`; both now emit the existing `/Admin/Events/Draft/{id}` route, proven by focused tests. | Resolved by the bounded route correction; no UI redesign or new abstraction was added. |
...
1. F-01 is resolved by the bounded Draft-route correction; retain its historical evidence and focused regression coverage;
