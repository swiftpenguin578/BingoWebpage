...
The first account journey after a successful Discord callback routes an incomplete profile through onboarding before committing an event signup. The user enters the exact OSRS account name they intend to use as the public username and chooses a required password. The UI explains that correct spelling is their responsibility. The server trims surrounding whitespace and normalizes case only for uniqueness; it does not call OSRS or Wise Old Man or validate syntax, availability, ownership, membership, or existence. It commits the unique public/login username, password hash, corresponding trust-based character link, preferred-account selection, and onboarding completion atomically. A database uniqueness conflict returns an accurate username-taken error without merging website accounts or character ownership.
...
Username changes select one of the account's active character links and reuse the normalized uniqueness constraint. The UI confirms that the new public username becomes the password-login username. The rename transaction updates participant display snapshots only for currently open signups. It does not mutate closed/historical participant snapshots or any event character assignments.
...
The signup form loads the authenticated account's trust-based OSRS character links and allows another link to be added. Global links are many-to-many and do not prove ownership. A partial unique `(event_id, osrs_character_id)` constraint over unreleased assignments protects current event reservations for both confirmed and waiting-list participants. The complete signup/create or edit command commits the participant, answers, any new global links, event assignments, EHB snapshots, form marker/version, and capacity/status result in one transaction. The server translates a uniqueness race into an account-specific “already assigned in this event” validation result, rolls back the whole attempt, and returns the other posted values for correction. A failed edit leaves the previously committed signup unchanged.

My accounts is an authenticated global account-management surface. Optional personal labels and ordering are presentation metadata, not game-mode or ownership assertions. Unlinking hides the association from future selection but does not cascade into historical event assignments, evidence, or activity.

Event character assignments distinguish `PLAYING` from `INFORMATIONAL`. The built-in required Account question and any additional playing Account questions create playing assignments. An informational Account question may record a named helper alt, while a Yes/No question records only its existence. Only playing accounts participate in active eligibility, evidence credit, or Wise Old Man synchronization. The built-in primary playing assignment is the planned initial active account and receives the initial activation at event start. Signup close freezes the assigned set and roles for participant editing, while live swaps remain available among the frozen playing accounts.

Each playing assignment stores its own signup-time EHB snapshot. Secondary Account questions are structurally optional, but when answered, playing-role validation requires EHB in the same compound control. EHB is not stored on the global My accounts link because it changes over time. Draft sorting and balancing use only the assignment produced by the built-in primary Account question; secondary values are neither summed nor substituted.
...
The public signup board uses explicit question visibility metadata and never exposes admin-only answers. New custom questions default public; the built-in primary playing account, its EHB, and captain-volunteer answer are always public. A secondary playing Account question exposes or hides its account/EHB pair together. Confirmed and waiting-listed profiles are shown in separate sections using their event public-username snapshots; waiting-listed profiles show exact derived positions. Informational Account answers are eligible only for that board and are excluded from later roster, evidence, progress, and leaderboard projections.
...
Legacy/imported records may temporarily use a claim or private-token migration path. Any retained tokens use high-entropy random values with only hashes stored in PostgreSQL, stop working when the record is claimed, and are removed after migration. Never auto-link by Discord display name or OSRS character name.
...
External/pre-formed teams may contain unlinked roster members. Normal submission access belongs only to claimed captain/co-captain Discord identities for that team; several captains may have separate identities. Explicitly enabled emergency captain credentials retain the same team scope.
...
Manual and scheduled opening call the same application/domain readiness policy with an explicit opening mode. Manual opening ignores any stored scheduled-opening value and supplies the current authoritative instant. Scheduled opening transactionally revalidates the complete event/form configuration at execution time; an invalid transition leaves the event closed, records the failed transition, and creates a durable admin-visible alert rather than partially publishing signup. External-provider reachability is not part of this transaction.
